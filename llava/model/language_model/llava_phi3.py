...
class LlavaPhi3Config(LlavaConfig):
    model_type = "llava_phi3"

## FIXME we will follow the convention to add a new class for CausalLM in the future
class LlavaPhi3Model(LlavaMetaModel, LlavaMetaForCausalLM, PreTrainedModel):
    config_class = LlavaPhi3Config
    main_input_name = "input_embeds"
    supports_gradient_checkpointing = True
    
    def __init__(self, config: LlavaPhi3Config = None, *args, **kwargs) -> None:
    ...
    def from_pretrained(
        ...
        **kwargs,
        ...
        return super(LlavaPhi3Model).from_pretrained(pretrained_model_name_or_path, 
            *model_args, config=config, cache_dir=cache_dir, ignore_mismatched_sizes=ignore_mismatched_sizes, force_download=force_download, local_files_only=local_files_only, token=token, 
            revision=revision, use_safetensors=use_safetensors, **kwargs)    
...
AutoModel.register(LlavaPhi3Config, LlavaPhi3Model)
